...
class RSM_EF:
    ...
    def solve_problem(self):
        ...
        while True:
            ...
            B = np.matmul(B, E)
            v = []
            # Calculating vectors v, w, ... using eta matrices
            for i in range(len(eta_matrices)):
                if i == 0:
            ...
            if np.count_nonzero(cn_p < 0) == 0:
                ...
                for w in range(len(v)):
                    print("v%d = %s" % (w, v[w]))
                print("u = %s" % u)
                print("f_opt = %s" % np.around((u.dot(self.b)*self.f_opt_sign), decimals=10))
                break
            ...
            print("f = %s" % np.around((u.dot(self.b)*self.f_opt_sign), decimals=10))
            # Creating new eta matrix and appending it to the list of all eta matrices
            E = np.eye(len(p))
            E[:, p.index(l)] = y[0, p]
...
def main():
    ...
    while ans == 'y':
        ...
        if system_form == 4:
            ...
            problem = RSM_EF(matrix_a, b[0], c, columns, f_opt_sign, system_form)
